...
export function registerGlobal<Type extends keyof OTelGlobalAPI>(
  ...
  const api = global[GLOBAL_OPENTELEMETRY_API_KEY] = global[
...
export function getGlobal<Type extends keyof OTelGlobalAPI>(
  ...
  const globalVersion = global[GLOBAL_OPENTELEMETRY_API_KEY]?.version;
  if (!globalVersion || !isCompatible(globalVersion)) {
    return;
  }
  return global[GLOBAL_OPENTELEMETRY_API_KEY]?.[type];
}
...
export function unregisterGlobal(type: keyof OTelGlobalAPI, diag: DiagLogger) {
  ...
  const api = global[GLOBAL_OPENTELEMETRY_API_KEY];
